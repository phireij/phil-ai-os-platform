const copy = {
  en: {
    ...
    heroCopy: "A first look at the customer-facing Ruby's Cake Delights storefront direction. The Initial Launch Catalog V1 scope is CEO-approved for progress; this page remains separated from production until publication content and launch acceptance are complete.",
    ...
    catalogNote: "These cards reflect the CEO-approved provisional three-product V1 scope. They remain preview-only and are not published to WooCommerce.",
  ...
  ja: {
    ...
    heroCopy: "Ruby's Cake Delights のお客様向けストアデザインの進捗プレビューです。初回公開カタログV1の対象はCEO承認済みですが、公開用コンテンツと最終受入が完了するまでは本番サイトから分離して表示しています。",
    ...
    catalogNote: "ここに表示しているのはCEO承認済みの暫定V1対象3商品です。プレビュー専用であり、WooCommerceには公開されていません。",

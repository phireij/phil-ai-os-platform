...
The current public AirREGI FAQ describes the data that can be linked through this API as:
...
> **AirREGI direct inventory API integration is UNPROVEN and must remain fail-closed until endpoint-level capability is confirmed.**

The public list of API-connected systems also presents a defined set of supported external systems rather than documenting a general-purpose public inventory API contract for arbitrary merchant-built clients.
...
- that CSV can be edited and uploaded to bulk-register/update inventory quantities;
- stocktake detail and summary CSV exports also exist;
...
That makes a deterministic AirREGI product/variation ↔ WooCommerce SKU mapping operationally realistic without inventing a second identifier scheme.
...
7. If stock writes are unsupported, can a supported transaction interface safely produce the needed AirREGI inventory movement, or is the documented API outbound/read-only from AirREGI's perspective?
...
## Candidate architecture A — only if supported inventory endpoints are verified
...
If direct inventory endpoints remain unavailable, the supported fallback is:
...
The CSV fallback is **not near-real-time**. Therefore it must use tighter safeguards than a direct API bridge, such as conservative safety stock, freshness limits, automatic Quick Pickup suspension when the latest inventory snapshot is stale, and explicit reconciliation after Woo sales.
...
**WooCommerce Quick Pickup remains feasible as a product feature.** The unresolved point is the mechanism for keeping AirREGI physical inventory authoritative with acceptable freshness and reconciliation risk.

The investigation now supports two materially different paths:
...
- Preserve AirREGI as the intended physical-stock authority unless a later evidence-based decision shows that a different architecture is safer.

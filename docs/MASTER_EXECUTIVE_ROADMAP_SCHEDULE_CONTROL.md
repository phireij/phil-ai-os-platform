...
| Last completed checkpoint | **Actual WooCommerce preproduction final-screen review completed RED / fail-closed, with root causes diagnosed read-only.** The product lacks a source-backed shipping class against a zero-base-cost zone; KOMOJU is connected as a test account with the approved four-method subset selected but its WooCommerce provider is globally inactive; and the already-published Tokushoho/cancellation pages are not linked from checkout. No order, payment or corrective site mutation was executed. |
...
| KOMOJU | Live dashboard/configuration evidence, the approved initial payment subset and Live Konbini 3-day expiry remain GREEN readiness facts. The actual WooCommerce preproduction provider is connected in test-account mode with Credit Card, Konbini, Merpay and Paidy selected, but is globally inactive; matching checkout exposure is therefore RED. No live or test payment execution is authorized. |
...
| Immediate next action | Obtain the missing product package/shipping-class fact (or a separately approved preproduction fallback), then—with action-time authorization—enable only the connected KOMOJU test-account provider and surface the two existing approved legal routes at checkout. Repeat the guarded non-submitted review; do not execute payment or place an order. |
...
**Status: CURRENT / actual checkout reviewed RED; configuration and disclosure root causes are now diagnosed read-only.**

Complete or confirm before Sprint 4 actual final-confirmation-screen acceptance:

- Resolve the missing package/shipping-class fact for Fudgy Milky Bar, or obtain explicit approval for a bounded preproduction fallback; do not infer a class. The current product has no class, while the applicable Frozen flat-rate method has base cost `0`, no no-class charge, and verified Yamato class charges.
- With action-time authorization, enable the already-connected KOMOJU **test-account** provider only. Credit Card, Konbini, Merpay and Paidy are already selected; the provider remains globally inactive. Verify Konbini's three-day deadline without executing payment.
- With action-time authorization, surface direct checkout links to the already-published cancellation/refund and Tokushoho pages. Do not change their approved text or broaden publication scope.

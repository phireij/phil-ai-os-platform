...
def build_transport() -> ProductionWooCommerceTransport:
    ...
    if not base_url:
        raise SystemExit("PHIL_AI_OS_WOO_READONLY_SNAPSHOT_BLOCKED: base_url_missing")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1:
        raise SystemExit("usage: tools_production_readonly_catalog_snapshot.py OUTPUT_JSON")

    output_path = Path(args[0])
    snapshot = collect_catalog_snapshot(build_transport())
    ...
    print(
        "PHIL_AI_OS_WOO_PRODUCTION_READONLY_CATALOG_SNAPSHOT_GREEN "
        f"products={len(snapshot.products)} categories={len(snapshot.categories)} "
        "network_read_only=true mutation=false production_publish=false"
    )
    return 0

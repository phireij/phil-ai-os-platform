...
def build_transport() -> ProductionWooCommerceTransport:
    ...
    if not base_url:
        raise SystemExit("PHIL_AI_OS_WOO_READONLY_SHIPPING_SNAPSHOT_BLOCKED: base_url_missing")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1:
        raise SystemExit("usage: tools_production_readonly_shipping_snapshot.py OUTPUT_JSON")

    output_path = Path(args[0])
    snapshot = collect_shipping_snapshot(build_transport())
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(
        json.dumps(snapshot.as_dict(), ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    methods = [method for zone in snapshot.zones for method in zone.methods]
    enabled = [method for method in methods if method.enabled]
    print(
        "PHIL_AI_OS_WOO_PRODUCTION_READONLY_SHIPPING_SNAPSHOT_GREEN "
        f"zones={len(snapshot.zones)} methods={len(methods)} enabled={len(enabled)} "

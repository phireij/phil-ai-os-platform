...
.card{background:#fff;border:1px solid #dbe2ea;border-radius:12px;padding:16px;box-shadow:0 1px 3px rgba(0,0,0,.06);min-width:0}
h2{font-size:16px;margin:0 0 12px}.kv{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.15fr);gap:8px;margin:6px 0;align-items:start}.kv strong{overflow-wrap:anywhere;text-align:right}.muted{color:#667085}.badge{padding:3px 8px;border-radius:999px;background:#eef2f6;font-size:12px}.warn{color:#9a6700}.err{color:#b42318}.ok{color:#067647}ul{padding-left:18px}button{display:none}.meta{font-size:12px;color:#667085}.value-meta{display:block;margin-top:2px;font-weight:400}.links{max-height:220px;overflow-y:auto;overflow-x:hidden;padding-right:4px}.links li{margin-bottom:8px;min-width:0}.approval-id{display:block;overflow-wrap:anywhere;word-break:break-word}
...
const kvp=(k,v,p)=>`<div class="kv"><span class="muted">${esc(k)}</span><strong>${esc(v)}${p?`<span class="meta value-meta">${esc(p)}</span>`:''}</strong></div>`;
fetch('api/read-model',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(d=>{
 ...
 document.getElementById('governance').innerHTML=kvp('Allowed classes',(g.execution_allowed_task_classes||[]).join(', '),gp.execution_allowed_task_classes)+kvp('Enforcement',g.execution_enforcement_mode,gp.execution_enforcement_mode)+kvp('Scope',g.execution_enforcement_scope,gp.execution_enforcement_scope)+kvp('Kill switch',g.kill_switch_state,gp.kill_switch_state)+kv('Authority expansion',g.authority_expansion_state)+kv('Direct provider bypass',g.direct_provider_bypass_allowed);
